...
Full search-space optimization: 9 origins × 10 departures × 16 returns = 1,440 searches
"""
import requests
import uuid
import time
...
POSITIONING_COSTS = {
    "AMD": 0, "BOM": 4000, "DEL": 6000, "PNQ": 4000,
    "BLR": 6000, "HYD": 5000, "MAA": 6000, "CCU": 7000, "GOI": 4000,
}

ENTITY_CACHE = {
    "AMD": "95673366", "BOM": "95673320", "DEL": "95673498",
    "MRU": "128668851", "BLR": "95673351", "PNQ": "128668941",
    "SIN": "95673375", "DXB": "95673506", "DOH": "95673852",
    "CDG": "95565041", "IST": "95673323", "LHR": "95565050",
    "JFK": "95565058", "NRT": "128668889",
}

ORIGINS = ["AMD", "BOM", "DEL", "PNQ", "BLR", "HYD", "MAA", "CCU", "GOI"]
DEST = "MRU"

DEP_DATES = [date(2026, 7, d) for d in range(1, 11)]  # Jul 1-10
RET_DATES = [date(2026, 7, d) for d in range(16, 32)]  # Jul 16-31
...
class Searcher:
    ...
    def search(self, origin, dest, dep, ret):
        ...
        body = {
            "cabinClass": "ECONOMY", "adults": 1, "childAges": [],
            "legs": [
                {"legOrigin": {"@type": "entity", "entityId": o_eid},
                 "legDestination": {"@type": "entity", "entityId": d_eid},
                 "dates": {"@type": "date", "year": str(dep.year),
                           "month": str(dep.month).zfill(2), "day": str(dep.day).zfill(2)}},
                {"legOrigin": {"@type": "entity", "entityId": d_eid},
                 "legDestination": {"@type": "entity", "entityId": o_eid},
                 "dates": {"@type": "date", "year": str(ret.year),
                           "month": str(ret.month).zfill(2), "day": str(ret.day).zfill(2)}},
            ],
        }
        ...
        try:
            ...
            polls = 0
            while status != "complete" and polls < 8:
                polls += 1
                time.sleep(2)
                try:
            ...
            for r in results:
                try:
                    ...
                    def carriers(leg):
                        return ", ".join(c.get("name", "") for c in leg.get("carriers", {}).get("marketing", []))
                    ...
                    flights.append(Flight(
                        ...
                        departure_date=str(dep), return_date=str(ret),
                        trip_days=trip_days,
                        price_raw=p, price_formatted=price.get("formatted", ""),
                        positioning_cost=pos_cost,
                        effective_total=p + pos_cost,
                        outbound_duration=od, return_duration=rd,
                        total_duration=od + rd,
                        outbound_stops=os_, return_stops=rs_, total_stops=ts,
                        outbound_carriers=carriers(ob), return_carriers=carriers(ib),
...
if __name__ == "__main__":
    ...
    # Phase 1: Resolve all entities
    print("\n--- Entity Resolution ---")
    for code in ORIGINS + [DEST]:
        eid = resolve_entity(code)
        if eid:
            print(f"  {code}: {eid}")
        else:
            print(f"  {code}: FAILED - removing from search")
            ORIGINS.remove(code) if code in ORIGINS else None
    
    # Phase 2: Calculate search space
    combos = list(product(ORIGINS, DEP_DATES, RET_DATES))
    total = len(combos)
    print(f"\n--- Search Space ---")
    print(f"  Origins: {len(ORIGINS)}")
    print(f"  Departures: {len(DEP_DATES)} ({DEP_DATES[0]} to {DEP_DATES[-1]})")
    print(f"  Returns: {len(RET_DATES)} ({RET_DATES[0]} to {RET_DATES[-1]})")
    ...
    for i, (origin, dep, ret) in enumerate(combos):
        ...
        if i % 10 == 0:
            ...
            print(f"\n  [{i+1}/{total}] ({pct:.0f}%) ETA: {eta/60:.0f}m | "
                  f"Flights: {len(all_flights)} | Fails: {searcher.fail_count}", flush=True)
        ...
        # Print individual result (compact)
        if flights:
            best = min(f.price_raw for f in flights)
            sys.stdout.write(f"  {origin}->{DEST} {dep.strftime('%m/%d')}-{ret.strftime('%m/%d')} "
                           f"{len(flights):3d}fl Rs.{best:,.0f}\n")
        else:
            sys.stdout.write(f"  {origin}->{DEST} {dep.strftime('%m/%d')}-{ret.strftime('%m/%d')} FAIL\n")
        sys.stdout.flush()
        
        # Minimal delay — API handles it fine
        time.sleep(random.uniform(0.5, 1.5))
        
        # Save checkpoint every 100 searches
        if (i + 1) % 100 == 0:
            ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            cp_path = os.path.join(results_dir, f"checkpoint_{ts}.json")
    ...
    print(f"\n\n  Search complete in {elapsed_total/60:.1f} minutes")
    ...
    # 1. Top 50 cheapest
    print("\n--- TOP 50 CHEAPEST ---")
    print(f"{'#':>3} {'Price':>12} {'Origin':>5} {'Depart':>10} {'Return':>10} {'Days':>4} {'Stops':>5} {'Airline':<25}")
    for i, f in enumerate(cheapest[:50]):
        print(f"{i+1:3d} {f.price_formatted:>12} {f.origin:>5} {f.departure_date:>10} "
              f"{f.return_date:>10} {f.trip_days:4d} {f.total_stops:5d} {f.outbound_carriers[:25]:<25}")
    
    # 2. Top 20 after positioning
    print("\n--- TOP 20 AFTER POSITIONING COSTS ---")
    print(f"{'#':>3} {'Fare':>10} {'Pos':>6} {'Total':>12} {'Origin':>5} {'Depart':>10} {'Return':>10} {'Stops':>5}")
    for i, f in enumerate(by_effective[:20]):
        print(f"{i+1:3d} {f.price_raw:>10,.0f} {f.positioning_cost:>6,} {f.effective_total:>12,.0f} "
              f"{f.origin:>5} {f.departure_date:>10} {f.return_date:>10} {f.total_stops:5d}")
    ...
    # 5. Cheapest from AMD
    amd = [f for f in cheapest if f.origin == "AMD"]
    if amd:
        c = amd[0]
    ...
    if cheapest:
        ...
        print(f"  {c.price_formatted} | {c.origin} | {c.departure_date} to {c.return_date}")
    ...
    for budget in [50000, 55000, 60000]:
        ...
        if under:
            ...
            for f in under[:5]:
                print(f"    Rs.{f.price_raw:,.0f} | {f.origin} | {f.departure_date}-{f.return_date} | "
                      f"{f.total_stops}stops | {f.outbound_carriers}")
        else:
            print(f"  None found")
    
    # 8. Price heatmap - minimum fare by dep/ret date
    print(f"\n--- PRICE HEATMAP (Min fare across all origins) ---")
    heatmap = {}
    for f in unique:
    ...
    ret_dates_str = sorted(set(f.return_date for f in unique))
    dep_dates_str = sorted(set(f.departure_date for f in unique))
    
    # Header
    hdr = f"{'Dep\\Ret':>12}"
    for rd in ret_dates_str:
        hdr += f" {rd[5:]:>8}"
    print(hdr)
    
    for dd in dep_dates_str:
        row = f"{dd[5:]:>12}"
        for rd in ret_dates_str:
    ...
    print(f"\n--- AIRPORT COMPARISON ---")
    print(f"{'Airport':>8} {'Min Fare':>12} {'Avg Fare':>12} {'Itineraries':>12} {'Min+Pos':>12}")
    for origin in sorted(ORIGINS):
        ...
        if flights_from:
            ...
            print(f"{origin:>8} {mn:>12,.0f} {avg:>12,.0f} {len(flights_from):>12,} {mn+pos:>12,.0f}")
    ...
    # CSV
    csv_path = os.path.join(results_dir, f"best_itineraries_{ts}.csv")
    fields = list(asdict(cheapest[0]).keys()) if cheapest else []
    ...
    # JSON
    json_path = os.path.join(results_dir, f"best_itineraries_{ts}.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "cheapest_from_amd": asdict(amd[0]) if amd else None,
    ...
    if unique:
        best_airport = min(ORIGINS, key=lambda o: min((f.price_raw for f in unique if f.origin == o), default=999999))
    ...
    if airline_avg:
        ...
        print(f"\n7. CHEAPEST AIRLINE (avg): {cheapest_airline} (avg Rs.{airline_avg[cheapest_airline]:,.0f})")
        print(f"   Top 5 airlines by average price:")
        for a in sorted(airline_avg, key=airline_avg.get)[:5]:
            print(f"     {a}: avg Rs.{airline_avg[a]:,.0f} ({len(airline_prices[a])} itineraries)")
    ...
    print(f"Runtime: {(time.time()-t_start)/60:.1f} minutes")
